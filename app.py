...
def admin_copy_script(uid):
    ...
    lines.append('}')
    lines.append('')
    lines.append('# ── Hard links por juego en carpeta temporal ─────────────────────')
    lines.append('# CreateHardLink (Win32) evita que PS interprete [ ] como wildcards.')
    lines.append('# Se crea una subcarpeta por juego para respetar la estructura que DBI espera.')
    ...
    lines.append('')
    lines.append('Write-Host "Preparando $($validos.Count) juegos..." -ForegroundColor Yellow')
    lines.append('foreach ($src in $validos) {')
    lines.append('    $nombre = Split-Path $src -Leaf')
    lines.append('    $gameDir = [System.IO.Path]::Combine($tempDir, $nombre)')
    lines.append('    [System.IO.Directory]::CreateDirectory($gameDir) | Out-Null')
    lines.append('    Get-ChildItem -LiteralPath $src -File -Recurse | ForEach-Object {')
    lines.append('        $rel  = $_.FullName.Substring($src.Length + 1)')
    lines.append('        $dest = [System.IO.Path]::Combine($gameDir, $rel)')
    lines.append('        $par  = [System.IO.Path]::GetDirectoryName($dest)')
    lines.append('        if (-not [System.IO.Directory]::Exists($par)) { [System.IO.Directory]::CreateDirectory($par) | Out-Null }')
    ...
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Enviando carpeta a DBI como un solo elemento..." -ForegroundColor Yellow')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('# .Self = la carpeta entera como un unico FolderItem — una sola operacion MTP')
    lines.append('$ns = $shell.Namespace($tempDir)')
    lines.append('if (-not $ns) { Write-Host "ERROR: no se pudo acceder a la carpeta temporal." -ForegroundColor Red; pause; exit }')
    lines.append('$sdFolder.CopyHere($ns.Self)')
    lines.append('')
    lines.append('Write-Host "Carpeta enviada. Supervisa el progreso en la pantalla de la Switch." -ForegroundColor Green')
    lines.append('Write-Host ""')
    lines.append('$ok = ""')
    lines.append('while ($ok -notmatch "^[sS]$") {')
    lines.append('    $ok = Read-Host "Escribe S y pulsa Enter cuando la Switch muestre 100% en todo"')
    lines.append('}')
