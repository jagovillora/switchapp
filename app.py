...
def admin_copy_script(uid):
    ...
    lines.append('}')
    lines.append('')
    lines.append('# ── Crear carpeta temporal con junctions y enviar todo de una vez ──')
    lines.append('$tempDir = Join-Path $env:TEMP "switch_install_$(Get-Random)"')
    ...
    lines.append('    $junctionPath = Join-Path $tempDir $nombre')
    lines.append('    if (-not (Test-Path -LiteralPath $junctionPath)) {')
    lines.append('        cmd /c "mklink /J `"$junctionPath`" `"$src`"" > $null')
    lines.append('    }')
    lines.append('}')
    lines.append('')
    lines.append('Write-Host "Enviando $($validos.Count) carpetas a DBI en una sola accion..." -ForegroundColor Yellow')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('$tempNS = $shell.Namespace($tempDir)')
    lines.append('$sdFolder.CopyHere($tempNS.Items())')
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Transferencia iniciada. Supervisa el progreso en la pantalla de la Switch." -ForegroundColor Green')
    lines.append('Write-Host "Cuando DBI termine, puedes borrar la carpeta temporal:" -ForegroundColor Gray')
    lines.append('Write-Host "  $tempDir" -ForegroundColor Gray')
    lines.append('Write-Host ""')
    lines.append('pause')
