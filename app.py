...
def admin_copy_script(uid):
    ...
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('# ── Hard links en el mismo disco que los juegos ──────────────────')
    lines.append('# Los ficheros se enlazan planos en $tempDir (sin subcarpetas por juego).')
    lines.append('# Asi CopyHere envia todo en UNA sola solicitud al MTP de la Switch.')
    ...
    lines.append('New-Item -ItemType Directory -Path $tempDir | Out-Null')
    lines.append('')
    ...
    lines.append('        $dest = Join-Path $tempDir $_.Name')
    lines.append('        New-Item -ItemType HardLink -Path $dest -Value $_.FullName -ErrorAction SilentlyContinue | Out-Null')
    lines.append('    }')
