...
def admin_copy_script(uid):
    ...
    lines.append('}')
    lines.append('')
    lines.append('Write-Host "Enviando $($validos.Count) juegos a DBI (sin ventanas de progreso)..." -ForegroundColor Yellow')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('# CopyHere con flag 4 (FOF_SILENT): suprime los dialogos de progreso de Windows')
    lines.append('# MTP solo acepta items individuales (.Self), no colecciones (.Items())')
    ...
    lines.append('    $sdFolder.CopyHere($shell.Namespace($src).Self, 4)')
    lines.append('}')
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Todos los juegos enviados a DBI." -ForegroundColor Green')
    lines.append('Write-Host "Supervisa el progreso en la pantalla de la Switch y cierra esta ventana cuando termine." -ForegroundColor Gray')
