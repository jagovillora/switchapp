...
def admin_copy_script(uid):
    ...
    lines.append('#    3. Ejecuta este script SIN Administrador')
    lines.append('#       (doble clic o clic derecho -> Ejecutar con PowerShell)')
    lines.append('# ================================================================')
    lines.append('')
    ...
    lines.append('')
    lines.append('# Comprobar que NO se ejecuta como admin (MTP no funciona elevado)')
    lines.append('$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)')
    lines.append('if ($isAdmin) {')
    lines.append('    Write-Host "ATENCION: Este script esta corriendo como Administrador." -ForegroundColor Red')
    lines.append('    Write-Host "Los dispositivos MTP (Switch) no son accesibles desde procesos elevados." -ForegroundColor Red')
    lines.append('    Write-Host "Cierra esta ventana y ejecuta el script SIN administrador (doble clic normal)." -ForegroundColor Yellow')
    ...
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('# ── Validar rutas ────────────────────────────────────────────')
    lines.append('$validos = [System.Collections.Generic.List[string]]::new()')
    lines.append('$errores = @()')
    lines.append('')
    lines.append('foreach ($juego in $juegos) {')
    lines.append('    $src = $juego.ruta')
    lines.append('    if ($juego.buscar) {')
    lines.append('        $encontrada = Get-ChildItem -LiteralPath $origenBase -Directory |')
    ...
    lines.append('    }')
    lines.append('    $validos.Add($src)')
    lines.append('}')
    lines.append('')
    lines.append('if ($errores.Count -gt 0) {')
    lines.append('    Write-Host ""')
    lines.append('    Write-Host "Rutas no encontradas ($($errores.Count)):" -ForegroundColor Red')
    lines.append('    $errores | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }')
    lines.append('    Write-Host ""')
    lines.append('}')
    lines.append('')
    lines.append('Write-Host "Enlazando $($validos.Count) juegos en carpeta temporal..." -ForegroundColor Yellow')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('# ── Hard links via Win32 API (evita que PS interprete [ ] como wildcards) ──')
    lines.append('Add-Type -TypeDefinition @\'')
    lines.append('using System;')
    lines.append('using System.Runtime.InteropServices;')
    lines.append('public class WinHardLink {')
    lines.append('    [DllImport("kernel32.dll", CharSet=CharSet.Unicode, SetLastError=true)]')
    lines.append('    public static extern bool CreateHardLink(string dst, string src, IntPtr sec);')
    lines.append('}')
    lines.append('\'@')
    lines.append('')
    lines.append('$tempDir = Join-Path $origenBase ("_switch_install_" + (Get-Random))')
    lines.append('[System.IO.Directory]::CreateDirectory($tempDir) | Out-Null')
    lines.append('')
    lines.append('$colisiones = 0')
    lines.append('foreach ($src in $validos) {')
    lines.append('    $nombre = Split-Path $src -Leaf')
    lines.append('    Get-ChildItem -LiteralPath $src -File -Recurse | ForEach-Object {')
    lines.append('        $dest = [System.IO.Path]::Combine($tempDir, $_.Name)')
    lines.append('        if ([System.IO.File]::Exists($dest)) {')
    lines.append('            Write-Warning "Nombre duplicado ignorado: $($_.Name) ($nombre)"')
    lines.append('            $colisiones++')
    lines.append('        } else {')
    lines.append('            [WinHardLink]::CreateHardLink($dest, $_.FullName, [IntPtr]::Zero) | Out-Null')
    lines.append('        }')
    lines.append('    }')
    lines.append('    Write-Host "  + $nombre" -ForegroundColor Cyan')
    lines.append('}')
    lines.append('if ($colisiones -gt 0) {')
    lines.append('    Write-Host "Aviso: $colisiones fichero(s) con nombre duplicado no se copiaran (ver advertencias arriba)." -ForegroundColor Yellow')
    lines.append('}')
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Iniciando copia..." -ForegroundColor Yellow')
    lines.append('Write-Host "La transferencia corre en segundo plano. Supervisa el progreso en la Switch." -ForegroundColor Gray')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('$sdFolder.CopyHere($shell.Namespace($tempDir).Items())')
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "========================================================" -ForegroundColor Yellow')
    lines.append('Write-Host " IMPORTANTE: La copia sigue en segundo plano." -ForegroundColor Yellow')
    lines.append('Write-Host " Espera a que la Switch muestre 100% en TODOS los juegos" -ForegroundColor Yellow')
    lines.append('Write-Host " antes de continuar. Si limpias antes, la copia fallara." -ForegroundColor Red')
    lines.append('Write-Host "========================================================" -ForegroundColor Yellow')
    lines.append('Write-Host ""')
    lines.append('$ok = ""')
    lines.append('while ($ok -notmatch "^[sS]$") {')
    lines.append('    $ok = Read-Host "Escribe S y pulsa Enter SOLO cuando la Switch muestre 100% en todo"')
    lines.append('}')
    lines.append('')
    lines.append('# Hard links: borrar las entradas del directorio NO borra los ficheros originales')
    lines.append('Remove-Item -LiteralPath $tempDir -Recurse -Force -ErrorAction SilentlyContinue')
    lines.append('Write-Host "Listo. Ficheros temporales eliminados." -ForegroundColor Green')
