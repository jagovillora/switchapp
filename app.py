...
def admin_copy_script(uid):
    ...
    lines.append('\'@')
    lines.append('')
    lines.append('$gamesDrive = Split-Path -Qualifier $origenBase')
    lines.append('$tempDir = Join-Path $gamesDrive ("_switch_install_" + (Get-Random))')
    ...
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Iniciando copia unica..." -ForegroundColor Yellow')
    lines.append('Write-Host "Supervisa el progreso en la pantalla de la Switch." -ForegroundColor Gray')
    lines.append('Write-Host ""')
    lines.append('')
    lines.append('$sdFolder.CopyHere($shell.Namespace($tempDir).Items())')
    lines.append('')
    lines.append('Write-Host ""')
    lines.append('Write-Host "Copia en curso. Cuando la Switch muestre 100% en todos los juegos," -ForegroundColor Green')
    lines.append('Write-Host "pulsa ENTER para limpiar los ficheros temporales y cerrar." -ForegroundColor Gray')
    lines.append('pause')
    lines.append('')
    lines.append('# Eliminar hard links — no borra los juegos originales, solo las entradas del directorio')
