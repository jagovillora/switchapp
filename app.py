...
def admin_copy_script(uid):
    ...
    lines.append('# ================================================')
    lines.append('')
    lines.append('# CONFIGURA SOLO EL DESTINO:')
    lines.append('$destino = "E:\\"   # Letra de tu microSD u otro destino')
    lines.append('')
    lines.append('# ================================================')
    lines.append('')
    ...
    if games_with_folder:
        lines.append('# ── Rutas exactas desde la biblioteca ────────────────')
        for g, folder in games_with_folder:
            ...
            dest_folder = folder.split('/')[-1].replace("'", "''")
            lines.append(f"# {g['display_name']} [{size}{dlc}]")
            lines.append(f"if (Test-Path '{src}') {{")
            lines.append(f"    Write-Host '  Copiando: {g['display_name']}' -ForegroundColor Cyan")
            lines.append(f"    robocopy '{src}' \"$destino\\{dest_folder}\" /E /NP /NFL /NDL | Out-Null")
            lines.append(f"    $copiados++")
            lines.append(f"}} else {{ Write-Warning '  No encontrado: {src}'; $errores += '{g['display_name']}' }}")
            lines.append('')

    if games_without_folder:
        lines.append('# ── Sin ruta guardada — busqueda por nombre ──────────')
        lines.append(f'$origenBase = "{BASE_ORIGEN}"')
        lines.append('')
        for g in games_without_folder:
            size = f"{g['size_mb']//1024}GB" if g['size_mb'] >= 1024 else (f"{g['size_mb']}MB" if g['size_mb'] else "?")
            dlc  = f" +{g['dlc_count']} DLC" if g['dlc_count'] else ""
            name = g['display_name'].replace("'", "''")
            lines.append(f"# {g['display_name']} [{size}{dlc}]")
            lines.append(f"$c = Get-ChildItem -Path $origenBase -Directory | Where-Object {{ $_.Name -like '*{name}*' }} | Select-Object -First 1")
            lines.append(f"if ($c) {{ Write-Host \"  Copiando: $($c.Name)\" -ForegroundColor Cyan; robocopy $c.FullName \"$destino\\$($c.Name)\" /E /NP /NFL /NDL | Out-Null; $copiados++ }}")
            lines.append(f"else {{ Write-Warning '  No encontrado: {name}'; $errores += '{name}' }}")
            lines.append('')
